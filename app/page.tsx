...
export default function HydroponicTowerApp() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("hydroCaru_Final_V8");
    if (saved) {
  ...
  useEffect(() => {
    if (isSetupComplete) {
      localStorage.setItem("hydroCaru_Final_V8", JSON.stringify({ isSetupComplete, plants, params, history }));
    }
        ...
        <Card className="w-full max-w-md p-8 bg-white rounded-[3rem] shadow-2xl space-y-6">
          ...
          {setupStep === 1 ? (
            <div className="space-y-4">
              <p className="text-center text-slate-400 font-bold uppercase text-xs">Paso 1: Configura el Agua</p>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <span className="text-[10px] font-black ml-2 uppercase text-slate-400">Litros</span>
                  <input type="text" value={params.waterVol} onChange={e => setParams({...params, waterVol: e.target.value})} className="w-full p-4 bg-slate-50 border-2 rounded-2xl text-center font-black" />
                </div>
                <div className="space-y-1">
                  <span className="text-[10px] font-black ml-2 uppercase text-slate-400">EC Agua</span>
                  <input type="text" value={params.ec} onChange={e => setParams({...params, ec: e.target.value})} className="w-full p-4 bg-slate-50 border-2 rounded-2xl text-center font-black" />
                </div>
              </div>
            ...
            <div className="space-y-4">
              <p className="text-center text-slate-400 font-bold uppercase text-xs">Paso 2: Plántulas Iniciales ({plants.length}/6)</p>
              <div className="grid grid-cols-3 gap-2">
                {[1,2,3,4,5,6].map(i => (
                  <div key={i} className={`aspect-square rounded-xl border-2 flex items-center justify-center ${plants.length >= i ? 'bg-green-500 text-white' : 'bg-slate-50 text-slate-200 border-dashed'}`}><Sprout /></div>
                ))}
              </div>
              ...
              <button disabled={plants.length < 6} onClick={() => setIsSetupComplete(true)} className="w-full bg-green-600 text-white p-5 rounded-2xl font-black disabled:opacity-30">EMPEZAR</button>
            </div>
          )}
        </Card>
      </div>
    ...
    <div className="min-h-screen bg-slate-50 pb-36 font-sans">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="overview" className="space-y-4">
            {mlNutrientes > 0 && (
              <Card className="bg-blue-600 text-white p-6 rounded-[2.5rem] shadow-xl flex items-center gap-6 animate-in zoom-in">
                ...
                <div><p className="text-[10px] font-black uppercase opacity-80">Nutrientes A+B</p><p className="text-4xl font-black">{mlNutrientes.toFixed(1)} ml</p></div>
              </Card>
            )}
            ...
            {phCorrection.ml > 0 && (
              <Card className={`${phCorrection.color} text-white p-6 rounded-[2.5rem] shadow-xl flex items-center gap-6 animate-in zoom-in`}>
                ...
                <div><p className="text-[10px] font-black uppercase opacity-80">{phCorrection.type}</p><p className="text-4xl font-black">{phCorrection.ml.toFixed(1)} ml</p></div>
              </Card>
            )}
          ...
          <TabsContent value="measure">
            <Card className="p-8 rounded-[3rem] space-y-6 shadow-2xl border-none">
              <h2 className="text-center font-black text-2xl italic">Nueva Medición</h2>
              <div className="space-y-4">
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-slate-400 ml-4 uppercase tracking-widest">pH</label>
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-slate-400 ml-4 uppercase tracking-widest">EC</label>
                ...
                <button onClick={saveEntry} className="w-full bg-slate-900 text-white p-7 rounded-[2rem] font-black text-xl shadow-lg">GUARDAR</button>
              </div>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="tower" className="space-y-6">
            {[1, 2, 3].map(lvl => (
              <div key={lvl} className="bg-white p-6 rounded-[2.5rem] shadow-sm border">
                <p className="text-center text-[10px] font-black text-slate-300 uppercase tracking-widest mb-6">Nivel {lvl}</p>
                  ...
                  {[1, 2, 3, 4, 5, 6].map(pos => {
                    ...
                    return (
                      <button key={pos} onClick={() => p ? setPlants(plants.filter(id => id.id !== p.id)) : setShowPlantSelector({lvl, pos})}
                        className={`w-full aspect-square rounded-[2rem] flex flex-col items-center justify-center border-2 transition-all shadow-sm ${p ? `${config?.bg} border-white shadow-md text-white` : 'bg-slate-50 border-dashed border-slate-200 text-slate-200'}`}>
          ...
          <TabsContent value="calendar" className="space-y-4">
            <Card className="p-8 rounded-[2.5rem] shadow-sm space-y-6">
              <h3 className="font-black text-xl text-center italic uppercase underline decoration-amber-400">Tomas Diarias</h3>
              <div className="space-y-3">
                {[ {t: "08:00", d: "Mañana", c: "bg-amber-100 text-amber-700"}, {t: "14:00", d: "Mediodía", c: "bg-blue-100 text-blue-700"}, {t: "21:00", d: "Noche", c: "bg-indigo-100 text-indigo-700"} ].map(item => (
                  <div key={item.t} className="flex items-center gap-4 p-5 bg-slate-50 rounded-3xl border">
                    <div className={`${item.c} px-3 py-2 rounded-xl font-black text-xs shadow-sm`}>{item.t}</div>
                    <p className="font-black text-xs uppercase text-slate-600">{item.d}</p>
                  </div>
                ))}
              </div>
            </Card>
          ...
          <TabsContent value="history" className="space-y-3">
            {history.map((h: any) => (
              <div key={h.id} className="bg-white p-5 rounded-3xl border shadow-sm flex justify-between items-center">
                <div><p className="text-[9px] font-black text-slate-300 uppercase leading-none mb-1">{h.date}</p><p className="font-black text-slate-800 text-lg">pH {h.pH} | EC {h.ec}</p></div>
          ...
          <TabsContent value="settings" className="py-10 text-center">
            <button onClick={() => {localStorage.clear(); window.location.reload();}} className="w-full bg-red-100 text-red-600 p-8 rounded-[2rem] font-black border-2 border-red-200 uppercase text-xs">Resetear App</button>
          </TabsContent>
        </Tabs>
      </main>
      ...
      {showPlantSelector && (
        <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-md z-[100] flex items-end justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-[3rem] p-10 space-y-4">
            ...
            <div className="grid grid-cols-1 gap-2">
              {Object.keys(VARIETY_CONFIG).map(v => (
                <button key={v} onClick={() => addPlant(v)} className={`w-full p-5 rounded-2xl font-black text-left flex justify-between items-center ${VARIETY_CONFIG[v].bg} text-white shadow-sm`}>
