    ...
    <div className="min-h-screen bg-slate-50 pb-32 font-sans">
      ...
      <main className="container mx-auto p-4 max-w-md">
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="overview" className="space-y-4">
            <div className="space-y-3">
              {alerts.length === 0 ? (
                ...
                alerts.map((alert, i) => (
                  <Card key={i} className={`${alert.color} text-white p-6 rounded-[2.5rem] flex items-center gap-6 shadow-lg border-none animate-in slide-in-from-top-2`}>
          ...
          <TabsContent value="calendar" className="text-center p-4">
          ...
          <TabsContent value="history" className="space-y-3">
             {history.map(h => (
               <Card key={h.id} className="p-5 rounded-3xl bg-white border font-black shadow-sm">
                 <p className="text-[8px] text-slate-400 mb-1 tracking-widest">{h.date}</p>
                 <div className="flex justify-between items-center"><span>pH {h.pH} | EC {h.ec}</span><Badge className="bg-slate-100 text-slate-600 border-none">{h.waterVol}L</Badge></div>
               </Card>
             ))}
          </TabsContent>

          <TabsContent value="settings" className="py-10">
             <button onClick={() => {localStorage.clear(); window.location.reload();}} className="w-full bg-red-100 text-red-600 p-8 rounded-[2.5rem] font-black uppercase text-xs">Reset Total</button>
          </TabsContent>
        </Tabs>
      </main>
