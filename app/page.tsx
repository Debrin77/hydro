...
import { 
  ...
  ArrowDownCircle, Droplets, Check, Gauge, Lock, Lightbulb, AlertTriangle, Termometer
} from "lucide-react"
...
export default function HydroponicTowerApp() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("hydroCaru_FINAL_PRO_V7");
    if (saved) {
  ...
  useEffect(() => {
    if (isSetupComplete) {
      localStorage.setItem("hydroCaru_FINAL_PRO_V7", JSON.stringify({
  ...
  const systemStatus = useMemo(() => {
    ...
    if (currentVol < maxVol && (maxVol - currentVol) >= 0.1) {
      alerts.push({ title: 'RELLENAR AGUA', val: `${(maxVol - currentVol).toFixed(1)} L`, desc: 'Añadir agua pura al depósito', color: 'bg-cyan-600', icon: <Droplets /> });
    }

    if (ph > 6.2) {
      const correction = (ph - 6.0) * currentVol * 1.5;
      alerts.push({ title: 'pH ALTO', val: `${correction.toFixed(0)} ml`, desc: 'Dosis de pH DOWN', color: 'bg-purple-600', icon: <ArrowDownCircle /> });
    } else if (ph < 5.6 && ph > 0) {
      const correction = (6.0 - ph) * currentVol * 1.5;
      alerts.push({ title: 'pH BAJO', val: `${correction.toFixed(0)} ml`, desc: 'Dosis de pH UP', color: 'bg-pink-600', icon: <ArrowDownCircle className="rotate-180" /> });
    }

    if (ec < targetEC && ec > 0) {
      const correctionNutri = ((targetEC - ec) / 0.1) * currentVol * 0.25;
      alerts.push({ title: 'EC BAJA', val: `${correctionNutri.toFixed(1)} ml`, desc: 'Añadir de Nutriente A y B', color: 'bg-blue-700', icon: <FlaskConical /> });
    } else if (ec > (targetEC + 0.3)) {
      const extraAgua = ((ec - targetEC) / targetEC) * currentVol;
      alerts.push({ title: 'EC ALTA', val: `${extraAgua.toFixed(1)} L`, desc: 'Diluir con agua pura inmediatamente', color: 'bg-red-600', icon: <AlertTriangle /> });
    }

    if (temp > 25) {
      alerts.push({ title: 'TEMP ALTA', val: `${temp}°C`, desc: 'Peligro de hongos. Añadir botellas de hielo.', color: 'bg-orange-600', icon: <AlertTriangle /> });
    } else if (temp < 15 && temp > 0) {
      alerts.push({ title: 'TEMP BAJA', val: `${temp}°C`, desc: 'Crecimiento lento. Revisar calefacción.', color: 'bg-blue-400', icon: <AlertTriangle /> });
    }
    ...
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
        <Card className="w-full max-w-md p-8 bg-white rounded-[3rem] shadow-2xl text-center text-slate-800">
          <div className="text-3xl font-[900] tracking-tight bg-gradient-to-r from-green-800 to-green-500 bg-clip-text text-transparent italic mb-8 pr-3">HydroCaru</div>
          {setupStep === 1 && (
            <div className="space-y-6">
              <h3 className="text-xl font-black uppercase text-slate-400">Capacidad (L)</h3>
          ...
          {setupStep === 2 && (
            <div className="space-y-6">
              <h3 className="text-xl font-black uppercase text-slate-400">Carga Nivel 1</h3>
              <LevelGrid lvl={1} />
              <button disabled={plants.length === 0} onClick={() => setSetupStep(3)} className="w-full bg-slate-900 text-white p-6 rounded-[2rem] font-black uppercase">Siguiente</button>
            </div>
          )}
          {setupStep === 3 && (
            <div className="space-y-6">
        ...
        {showPlantSelector && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[9999] flex items-end p-4">
            <div className="bg-white w-full max-w-md mx-auto rounded-[3.5rem] p-10 animate-in slide-in-from-bottom">
              <h3 className="text-center font-black uppercase text-xs mb-8 text-slate-400 text-slate-800">Seleccionar Variedad</h3>
    ...
    <div className="min-h-screen bg-slate-50 pb-32 text-slate-800">
      <header className="bg-white border-b p-6 flex justify-between items-center sticky top-0 z-50">
        <div className="flex items-center gap-3">
           <div className="text-2xl font-[900] tracking-tight bg-gradient-to-r from-green-800 to-green-500 bg-clip-text text-transparent italic pr-4">
             HydroCaru
           </div>
        </div>
        <Badge className="bg-slate-900 text-white font-black px-4 py-1 rounded-full text-xs">{params.waterVol}L / {initialVol}L</Badge>
      </header>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="overview" className="space-y-4">
            ...
            {systemStatus.alerts.length === 0 && (
              <Card className="p-10 rounded-[2.5rem] bg-green-50 text-green-700 border-2 border-green-100 flex flex-col items-center justify-center gap-2">
                <Check size={40}/><span className="font-black uppercase text-sm italic tracking-widest">Sistema Óptimo</span>
              </Card>
            )}
          </TabsContent>
          ...
          <TabsContent value="measure">
            <Card className="p-8 rounded-[3rem] bg-white shadow-2xl space-y-6 border-none">
              ...
              <button onClick={() => { setHistory([{...params, id: Date.now(), date: new Date().toLocaleString()}, ...history]); setActiveTab("overview"); }} className="w-full bg-slate-900 text-white p-7 rounded-[2.2rem] font-black uppercase shadow-xl mt-4 active:scale-95 transition-all">Guardar Mediciones</button>
            </Card>
          </TabsContent>

          <TabsContent value="tower" className="space-y-6">
            {[1, 2, 3].map(lvl => (
              <div key={lvl}><p className="text-[10px] font-black text-slate-400 uppercase mb-2 ml-4 tracking-widest italic text-slate-800">Nivel {lvl}</p><LevelGrid lvl={lvl} /></div>
            ))}
            <Card className="p-8 rounded-[2.5rem] bg-slate-900 text-white text-center shadow-xl relative overflow-hidden">
                ...
                <button onClick={() => { if(confirm('¿Confirmar rotación?')) { const r = plants.filter(p => p.level !== 3).map(p => ({ ...p, level: p.level + 1 })); setPlants(r); setLastRotation(new Date().toISOString()); } }} className="mt-6 w-full bg-green-600 px-6 py-4 rounded-2xl font-black uppercase text-xs shadow-lg">Rotar Hoy</button>
             </Card>
          </TabsContent>

          <TabsContent value="history" className="space-y-4">
             {/* SECCIÓN DE PLANIFICACIÓN SEMANAL */}
             <Card className="p-6 rounded-[2.5rem] bg-indigo-900 text-white shadow-xl">
                <div className="flex items-center gap-3 mb-4"><CalendarIcon size={20} className="text-indigo-300"/><h3 className="font-black uppercase text-xs tracking-widest">Plan de Seguimiento</h3></div>
                <div className="grid grid-cols-3 gap-2">
                   {['Lunes', 'Miércoles', 'Viernes'].map(dia => (
                     <div key={dia} className="bg-indigo-800/50 p-3 rounded-2xl border border-indigo-700 text-center">
                        <p className="text-[8px] font-black uppercase opacity-60 mb-1">{dia}</p>
                        <p className="text-[10px] font-bold">MEDICIÓN</p>
                     </div>
                   ))}
                </div>
                <p className="text-[9px] mt-4 opacity-60 text-center italic">Recomendado: Medir siempre a la misma hora (Mañana)</p>
             </Card>

             <h2 className="text-xl font-black italic text-slate-800 ml-2 mt-6 uppercase tracking-tight text-slate-800">Bitácora de Registros</h2>
             {history.map(h => (
                <Card key={h.id} className="p-5 rounded-[2rem] bg-white border-2 font-black shadow-sm flex flex-col gap-2 animate-in slide-in-from-bottom-2 text-slate-800">
                  ...
                  <div className="flex justify-around text-xs italic">
                    <span className="flex items-center gap-1 text-slate-800">pH {h.pH}</span>
                    <span className="flex items-center gap-1 text-emerald-600">EC {h.ec}</span>
                    <span className={`${parseFloat(h.temp) > 25 ? 'text-red-500' : 'text-orange-400'}`}>{h.temp}°C</span>
                  </div>
                </Card>
             ))}
          </TabsContent>
          ...
          <TabsContent value="tips" className="space-y-4 pb-10">
            <h2 className="text-xl font-black italic text-slate-800 ml-2 mb-4 uppercase tracking-tighter text-center">Sabiduría Maestro</h2>
            <Card className="p-6 rounded-[2.5rem] bg-white border-2 border-green-100 shadow-sm leading-relaxed text-[11px] font-medium text-slate-600">
              ...
              Lava el cepellón en agua templada. Agita suavemente. ¡No tires! Una gota de agua oxigenada elimina patógenos.
            </Card>
            <Card className="p-6 rounded-[2.5rem] bg-slate-900 text-white shadow-xl">
              <div className="flex items-center gap-3 mb-4 text-yellow-400 font-black uppercase text-[10px] tracking-tighter"><Activity size={18}/> Tabla Maestra</div>
              <div className="grid grid-cols-3 text-[10px] font-bold border-t border-slate-700 pt-3 gap-y-3 uppercase tracking-tighter">
                ...
                <span className="text-yellow-400 italic font-black">PUNTO DULCE</span><span className="text-center text-yellow-400">6.0</span><span className="text-right text-yellow-400">1.4</span>
              </div>
            </Card>
          </TabsContent>

          <TabsContent value="settings" className="py-10">
             <button onClick={() => {if(confirm('¿BORRAR TODO?')) {localStorage.removeItem("hydroCaru_FINAL_PRO_V7"); window.location.reload();}}} className="w-full bg-red-100 text-red-600 p-10 rounded-[3rem] font-black uppercase text-xs tracking-widest border-2 border-red-200">Borrar Datos App</button>
             <p className="text-center mt-6 text-[8px] font-black text-slate-300 tracking-widest uppercase italic">HydroCaru v7.0 Full Precision</p>
          </TabsContent>
        </Tabs>
      </main>
